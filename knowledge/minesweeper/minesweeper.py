    ...
    def add_knowledge(self, cell, count):
        ...
        for sentence in self.knowledge:
            for other in self.knowledge:
                if sentence.cells.issubset(other.cells) and sentence != other:
                    new = Sentence(other.cells - sentence.cells,
                                   other.count - sentence.count)
                    if new not in new_knowledge:
                        new_knowledge.append(new)

        # remove empty sentences
        for i in range(len(new_knowledge) - 1, -1, -1):
            if len(new_knowledge[i].cells) == 0:
                new_knowledge.pop(i)
